...
public final class RatPoly {
  ...
  private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
	  for (int i = 0; i < lst.size(); i++) {
		  RatNum scaledCoeff = lst.get(i).getCoeff().mul(scalar);
		  RatTerm scaled = new RatTerm(scaledCoeff, lst.get(i).getExpt()); 
		  lst.set(i, scaled); 
	  } 
  }
  ...
  private static void incremExpt(List<RatTerm> lst, int degree) {
	  for (int i = 0; i < lst.size(); i++) {
		  RatTerm incr = new RatTerm(lst.get(i).getCoeff(), lst.get(i).getExpt() + degree);
		  lst.set(i, incr); 
	  } 
  }
  ...
  private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
	  ...
	  if (!newTerm.isZero()) {
		  if (lst.isEmpty()) {
			  lst.add(newTerm);
		  } else if (degree > lst.get(0).getExpt()) {
			  lst.add(0, newTerm);
		  } else if (degree < lst.get(lst.size() -1).getExpt()) {
			  lst.add(newTerm); 
		  } else {
			  for (int i = 0; i < lst.size() - 1; i++) {
				  int deg1 = lst.get(i).getExpt();
				  int deg2 = lst.get(i+1).getExpt(); 
				  if (deg1 > degree && degree > deg2) {
				  lst.add(i + 1, newTerm);
				  break; 
				  }
			  }
  ...
  public RatPoly negate() {
	  ArrayList<RatTerm> lst = new ArrayList<RatTerm>(); 
	  for (RatTerm t: this.terms) {
		  lst.add(new RatTerm(t.getCoeff().negate(), t.getExpt()));
	  }
  ...
  public RatPoly add(RatPoly p) {
	  if (this.isNaN() | p.isNaN()) {
		  return RatPoly.NaN;
	  } else if (p.equals(this.negate())) {
		  return RatPoly.ZERO; 
	  } else {
		  ArrayList<RatTerm> r = new ArrayList<RatTerm>();
		  for (RatTerm t: this.terms) {
			  r.add(t);
		  }
		  for (RatTerm tp: p.terms) {
			  int degree = tp.getExpt();
			  boolean found = false; 
			  for (int i = 0; i < r.size(); i++) {
				  if (r.get(i).getExpt() == degree) {
					  RatNum addedCoeff = r.get(i).getCoeff().add(tp.getCoeff());
					  RatTerm added = new RatTerm(addedCoeff, tp.getExpt());
					  if (added.isZero()) {
						  r.remove(i);
					  } else {
						  r.set(i, added); 
  ...
  public RatPoly mul(RatPoly p) {
	  if (this.isNaN() | p.isNaN()) {
		  ...
		  for (RatTerm tp: p.terms) {
			  ...
			  scaleCoeff(q, tp.getCoeff());
			  incremExpt(q, tp.getExpt());  
			  r = r.add(new RatPoly(q)); 
		  } 
		  return r; 
	  } 
  }
  ...
  public RatPoly div(RatPoly p) {
	  if (p.terms.isEmpty() | p.isNaN() | this.isNaN()) {
		  ...
		  while(r.terms.size() > 0 && r.terms.get(0).getExpt() >= p.terms.get(0).getExpt()) {
			  RatTerm tq = r.terms.get(0).div(p.terms.get(0));
			  RatPoly tqPoly = new RatPoly(tq);
			  q = q.add(tqPoly); 
			  RatPoly s = p.mul(tqPoly).negate();
			  r = r.add(s);
		  } 
		  return q;
	  }
  ...
  public RatPoly differentiate() {
	  	if (this.isNaN()) {
	  		...
	  		RatPoly totaldif = new RatPoly();
	  		for(RatTerm t: this.terms) {
	  			RatTerm dif = t.differentiate();
	  			totaldif = totaldif.add(new RatPoly(dif));
	  		}
	  		return totaldif; 
	  	}
  }
  ...
  public RatPoly antiDifferentiate(RatNum integrationConstant) {
	  if (this.isNaN()) {
	  		...
	  		RatPoly totalantidif = new RatPoly();
	  		for(RatTerm t: this.terms) {
	  			RatTerm antidif = t.antiDifferentiate();
	  			totalantidif = totalantidif.add(new RatPoly(antidif));
	  		}
	  		RatPoly constant = new RatPoly(new RatTerm(integrationConstant, 0)); 
	  		totalantidif = totalantidif.add(constant);
	  		return totalantidif; 
	  	}
  }
  ...
  public double integrate(double lowerBound, double upperBound) {
	  double upper = this.antiDifferentiate(RatNum.ZERO).eval(upperBound);
	  double lower = this.antiDifferentiate(RatNum.ZERO).eval(lowerBound);		  
	  return upper - lower; 
  }
