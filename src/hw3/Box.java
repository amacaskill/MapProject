...
public class Box implements Iterable<Ball> {
    ...
    private TreeSet<Ball> sortedBalls; 
    ...
    public Box(double maxVolume) {
    	...
    	sortedBalls = new TreeSet<Ball>(new BallComparator());
    }
    ...
    public boolean add(Ball b) {
    	boolean canAdd = maxVolume - ballContainer.getVolume() >= b.getVolume() 
    			         && !ballContainer.contains(b); 
    	if (canAdd) {
    		ballContainer.add(b);
    		sortedBalls.add(b); 
    	}  	
    ...
    public Iterator<Ball> getBallsFromSmallest() {
    	return Collections.unmodifiableSortedSet(sortedBalls).iterator();
    }
    ...
    private class BallComparator implements Comparator<Ball> {

		@Override
		public int compare(Ball b1, Ball b2) {
			double v1 = b1.getVolume();
			double v2 = b2.getVolume(); 
			if (v1 >= v2) {  // balls w/ v1=v2 are != 
				return 1; 
			} else  {  // (v1 <= v2)
				return -1; 
			}
